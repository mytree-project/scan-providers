...
async function main() {
    ...
    try {
        ...
        result = await fetchUnitOrdinalImage(context, page);
    } catch (error) {
        failure = error;
...
async function fetchUnitOrdinalImage(context, page) {
    const unitId = unitIdFromUrl(targetUrl);
    if (unitId === null) {
        fail(`Unit-scan browser flow received an unsupported unit URL: ${targetUrl}`);
    }

    debugEvent('unit:navigate:start', { url: targetUrl, unit_id: unitId });
    ...
    const targetPageNumber = Math.floor((scanNumber - 1) / UNIT_PAGE_SIZE) + 1;
    const indexOnPage = (scanNumber - 1) % UNIT_PAGE_SIZE;
    ...
    if (indexOnPage >= thumbnailCount) {
        fail(
            ...
            + `cannot select scan ${scanNumber} at page offset ${indexOnPage + 1}.`,
        );
    }
    ...
    if (displayedOrdinal !== null && Number.isInteger(displayedOrdinal) && displayedOrdinal !== scanNumber) {
        fail(
            `Szukaj w Archiwach scan ordering mismatch: requested ${scanNumber}, `
            + `but the selected thumbnail is labeled ${displayedOrdinal}.`,
        );
    }
    if (objectId === null || !/^[1-9]\d*$/.test(objectId)) {
        fail(`Selected scan ${scanNumber} does not expose a valid data-plikid.`);
    }
    if (objectId !== expectedObjectId) {
        fail(
            `Resolved object mismatch for scan ${scanNumber}: catalog resolved ${expectedObjectId}, `
    ...
    await waitForPhotoSliderApplication(frame);
    ...
    return await fetchDirectViewerImage(viewerPage, publicViewerUrl);
}
...
async function waitForPhotoSliderFrame(page, objectId) {
    ...
    debugEvent('unit:photoslider-missing', {
        object_id: objectId,
...
async function waitForPhotoSliderApplication(frame) {
    const waitTimeout = Math.min(timeoutMs, 20000);
    ...
    const elementCount = await frame.locator('body *').count().catch(() => 0);
    debugEvent('unit:photoslider-rendered', {
        frame_url: frame.url(),
    ...
    if (bodyText.trim() === '' && elementCount === 0) {
        fail('Szukaj w Archiwach photoslider iframe loaded, but its application did not render any content.');
    }
}
...
async function extractPublicViewerUrlFromPhotoSlider(frame) {
    ...
    while (Date.now() < deadline && trigger === null) {
        const textCandidate = frame
            .getByText(/^\s*Link do s[ck]anu\s*$/i, { exact: true })
            .first();
        if (await textCandidate.count() > 0 && await textCandidate.isVisible().catch(() => false)) {
            trigger = textCandidate;
            break;
        }

        const attributeCandidate = frame
            .locator('[aria-label], [title]')
            .filter({ has: frame.locator('*') });
        const attributeCount = await attributeCandidate.count().catch(() => 0);
        for (let index = 0; index < attributeCount; index += 1) {
            const candidate = attributeCandidate.nth(index);
            ...
            if (/^\s*Link do s[ck]anu\s*$/i.test(ariaLabel ?? '') || /^\s*Link do s[ck]anu\s*$/i.test(title ?? '')) {
                trigger = candidate;
                break;
            }
        }
    ...
    if (trigger === null) {
        ...
        fail('Szukaj w Archiwach photoslider does not expose the "Link do scanu" control.');
    }
    ...
    while (Date.now() < deadline) {
        ...
        for (let index = 0; index < count; index += 1) {
            const value = await controls.nth(index).inputValue().catch(() => '');
    ...
    fail('Szukaj w Archiwach "Link do scanu" panel did not expose a recognizable public scan viewer URL.');
}
...
async function fetchDirectViewerImage(page, viewerUrl) {
    const candidates = [];
    const pending = [];
    ...
    if (candidates.length === 0) {
        fail(`Scan viewer did not load a recognized image from ${PHOTO_HOST}.`);
    }
...
function imageQualityRank(url) {
    try {
        ...
        if (pathname.endsWith('_max')) return 3;
        if (pathname.endsWith('_mid')) return 2;
        if (pathname.endsWith('_min') || pathname.endsWith('_thumb')) return 1;
    } catch {
        return 0;
    }
    return 0;
...
function validateArguments() {
    if (action !== 'unit-scan-image') {
        fail('Expected action unit-scan-image.');
    }
    ...
    if (expectedObjectId === null || !/^[1-9]\d*$/.test(expectedObjectId)) {
        fail('unit-scan-image action requires --expected-object-id=<positive integer>.');
    }
